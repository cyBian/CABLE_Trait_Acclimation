...
!    IF (ACCESS_run) THEN
!      ALLOCATE( r4dum(nlon, nlat, npatch, 1) )
...
!    END IF
   ...
   IF (icycle > 0) THEN
      ...
      ! x.zhang 2017/10/21
      inNdep = 0. !! when no Ndep effect!
      IF (casafile%l_ndep)THEN  !! read Ndep from Ndep file
      ...
      ELSE !! read Ndep from CSIRO gridinfo file
...
! Code if using UM soil file (not needed now since
...
!    DEALLOCATE(in2alb,sfact,dummy2,indummy)
  ...
  !   landpt%type    - via cable_IO_vars_module (%nap,cstart,cend,ilon,ilat)
    ...
    DO kk = 1, mland
      ...
      ELSE
        IF (nmetpatches == 1) THEN
          ! assume nmetpatches to be 1 for GSWP and GPCC runs
          ncount = ncount + 1
          landpt(kk)%nap = 1
          landpt(kk)%cend = ncount
    ...
    IF (max_vegpatches /= nmetpatches) THEN
      ...
      PRINT *, 'it really has. Check met file if not ACCESS runs.'
    END IF
...
!    IF(ASSOCIATED(soiltype_metfile)) DEALLOCATE(soiltype_metfile)
  ...
  SUBROUTINE derived_parameters(soil, sum_flux, bal, ssnow, veg, rough, met)
    ...
    CALL point2constants(C)  !added by BP Dec 2014
