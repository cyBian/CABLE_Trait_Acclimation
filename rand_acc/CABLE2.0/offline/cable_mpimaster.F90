...
!  !!! added by xzhang 09/01/2015
   ...
   IF (ncciy /= 0) THEN
     ...
     IF (globalMetfile%l_gpcc)THEN
       ...
       IF (ncciy < 1901 .OR. ncciy > 2030) THEN
...
!  CALL MPI_Bcast (mstype, 1, MPI_INTEGER, 0, comm, ierr)
  ...
  WRITE (*,*) 'second ntyp: ',ntyp,' nparam:',ncasaparam
  ntyp = ncasaparam
  WRITE (*,*) 'third ntyp: ',ntyp,' nparam:',ncasaparam
  ...
  blen(bidx) = mvtype * extr2
  !write(*,*) 'master-sla:bidx,mvtype,extr2',bidx,mvtype,extr2,mvtype*extr2,ntyp

  ! Added by Bian for grid-based sla (17Apr2024)
  bidx = bidx + 1
  CALL MPI_Get_address (casabiome%sla_grid, displs(bidx), ierr)
  blen(bidx) = r2len 
  !write(*,*) 'master:sla_grid:bidx,r2len,ntyp',bidx,r2len,ntyp
  ! -------------- End added -----------------------------
...
!==================================================================
  ...
  ! ====== Add a new variable casapool%nleaf by Bian 20Apr2024 =======
  bidx = bidx + 1
  CALL MPI_Get_address (casapool%nleaf(off), displs(bidx), ierr)
  blen(bidx) = r2len
  ! ==================== End added ===================================
  ...
  ! MPI: sanity check
  IF (bidx /= ntyp) THEN
     WRITE (*,*) 'master: invalid number of casa_t param fields ',bidx,'ntyp:',ntyp,' fix it!'
     CALL MPI_Abort (comm, 1, ierr)
  END IF
