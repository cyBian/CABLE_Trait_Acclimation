...
SUBROUTINE open_met_file(dels,kstart,kend,spinup, TFRZ )
   ...
   LOGICAL                          ::                                         &
        L_mask,  &  ! mask in met file or not   
...
!    ok = NF90_OPEN(globalMetfile%CO2air,0,ncid_co2) !!add by xzhang 20/01/2015
  ...
  ENDIF  ! added by xzhang 1/12/2014
    ...
    IF(ok /= NF90_NOERR) THEN
       ...
       IF(ok /= NF90_NOERR) THEN
         ...
         //TRIM(filename%met)//' (SUBROUTINE open_met_file)')
       ELSE
         lat_all = SPREAD(REAL(temparray1),1,xdimsize)
       END IF
    ...
    IF(ok /= NF90_NOERR .or. globalMetfile%l_ncar) THEN  !!changed by xzhang 1/12/2014
       ...
       IF(ok /= NF90_NOERR) THEN 
         ...
         //TRIM(filename%met)//' (SUBROUTINE open_met_file)')
       ELSE 
         lon_all = SPREAD(REAL(temparray1),2,ydimsize)
       END IF
       DEALLOCATE(temparray1)
    ELSE
       ! Needed since r_1 will be double precision with -r8:
    ...
    ELSE ! i.e. "pftmask" variable exists
       L_mask = .TRUE.
    END IF
    ...
    END IF ! "mask" variable or "pftmask" variable
...
!! end hacking
    ...
    ! ************ fixing NCAR met file time attribute ***********
    IF (ncciy > 0 .AND. globalMetfile%l_ncar) THEN
      READ(timeunits(12:15),*) tsyear
      syear = syear + tsyear
      timeunits = 'seconds since 1901-01-01 00:00:00'
      WRITE(timeunits(15:18),'(i4.4)') syear
    ENDIF
    !****** done bug fixing for NCAR time attribute ***********
...
!            /3600.0+shod)/24.0)+sdoy)/365.0)+syear
 ...
 !     IF (filename%CO2air .ne. '')THEN
 !       IF (ncciy > 0) ncid_met = ncid_co2
 !       print *,'reading from CO2air data'
 !     END IF
    ...
    IF (globalMetfile%l_ncar) Then
      ok = NF90_INQ_VARID(ncid_met,'SNOW',id%Snowf)
    ELSE ! gswp and single sites
   ...
   !    IF(ANY(laidimids==patchdimID)) THEN
    ...
    END IF
       ...
       print*,"exists%LAI,exists%LAI_T,exists%LAI_M,exists%LAI_P"
       print*,exists%LAI,exists%LAI_T,exists%LAI_M,exists%LAI_P
       print*,"exists%Snowf",exists%Snowf,"ncid_snow",ncid_snow
     IF (ncciy > 0 .AND. (globalMetfile%l_gswp .OR. globalMetfile%l_ncar))Then
       ncid_met = ncid_snow
     ELSE
       ncid_met=ncid_lw
...
!       IF(ktau==1) THEN ! initialise...
    ...
    IF(metGrid=='mask') THEN
      ...
      ! NCAR also used one met file only.
    ...
    ELSE IF(metGrid=='land') THEN
      ...
      ENDDO
  !    print*,"In get_met_data xzhang 2015.01.23"
  !    print*,"exists%Snowf",exists%Snowf
