...
known_hosts()
{
   set -A kh vayu cher burn shin jigg loca
}
...
host_loca()
{
   export NCDIR='//opt/envs/envs_cable/softwares/netcdf_intel/lib'
   export NCMOD='//opt/envs/envs_cable/softwares/netcdf_intel/include'
   export FC=ifort
   export CFLAGS='-O2 -fp-model precise'
   export LD='-lnetcdf -lnetcdff'
   export LDFLAGS='-L//opt/envs/envs_cable/softwares/netcdf_intel/lib -O2'
   build_build
   cd ../
   build_status
}
...
host_burn()
{
   ...
   export CFLAGS='-O2 -fp-model precise'
   if [[ $1 = 'debug' ]]; then
      export CFLAGS='-O0 -traceback -g -debug all -fp-model precise'
   fi
