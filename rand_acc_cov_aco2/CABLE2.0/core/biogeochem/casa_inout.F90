...
!  INTEGER,               INTENT(IN)    :: mvt,mst
  ...
  ! added by Bian for randomly SLA and Nleaf (17Apr2024)
  REAL(r_2), DIMENSION(mp)           :: xsla,xnleaf
...
!     PRINT *, 'nv10',nv10
  ...
  !======== added by Bian 17Apr2024, Read casafile%cnptrait =================
  OPEN(102,file=casafile%cnptrait)
  DO npt=1, mp
     READ(102,*) xsla(npt),xnleaf(npt)
  ENDDO
  write(*,*) 'casafile%cnptrait successfully read.'
  write(*,*) xsla(1:5), xnleaf(1:5)
  CLOSE(102)
  != ======= END of READ casafile%cnptrait file ============================
...
!  PRINT *, 'casabiome%ivt2 = ', casabiome%ivt2
  ...
  DO npt = 1, mp
    ...
    ! add by Bian for grid-based SLA and Na (17Apr2024)
    casabiome%sla_grid(npt) = xsla(npt)
...
!    PRINT *, 'npt,iv1,iso ', npt,iv1, iso
    ...
    ! replace initial glai with grid-based random SLA by Bian 17Apr2024

    ! casamet%glai(npt) = MAX(casabiome%glaimin(iv1), &
    !                    casabiome%sla(iv1) * casapool%cplant(npt,leaf))
    casamet%glai(npt) = MAX(casabiome%glaimin(iv1), &
                        casabiome%sla_grid(npt) * casapool%cplant(npt,leaf))

    casamet%glai(npt) = MIN(casabiome%glaimax(iv1),casamet%glai(npt))   ! added by BP Dec2014
...
!    casaflux%fNminleach(npt)  = xfNminleach(iv1) 
    ...
    ! Comment by Bian 17Apr2024
    !casapool%nplant(npt,leaf) = nleaf(iv1)
    casapool%nplant(npt,leaf) = xnleaf(npt)
    casapool%nleaf(npt) = xnleaf(npt) !Bian 20Apr2024
...
!    casaflux%Nminfix(npt)     = xnfixrate(iv1)/365.0  
  ...
  ! test for grid-based random SLA and Nleaf 
  write(*,*) 'casabiome%sla_grid(1:5):', casabiome%sla_grid(1:5)
  write(*,*) 'casapool%nplant(1:5,leaf):', casapool%nplant(1:5, leaf)
...
!  TYPE (veg_parameter_type), INTENT(IN) :: veg
  ...
  CALL readpar(ncid,'CASA_Cplant',dummy,casapool%Cplant,filename%restart_in, &
                max_vegpatches,'cnp',from_restart,mp)
!!changed by xzhang for modis-lai init cpool simulation!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 !!             casapool%Cplant=casapool%Cplant*0.92
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CALL readpar(ncid,'Nplant',dummy,casapool%Nplant,filename%restart_in, &
                max_vegpatches,'cnp',from_restart,mp)
! ============ used observed-based nleaf to replace the one in restart file ==========
  casapool%Nplant(:,leaf) = casapool%nleaf
! ================== end added by Bian 20Apr, 2024 =================================
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  CALL write_ovar(ncid, laiID, 'LAI', casamet%glai, ranges%lai, &
                  .TRUE., 'cnp', .TRUE.)
  ! replace PFT-based sla with grid-based sla by Bian 17Apr2024
  ! dummy(:) = casabiome%sla(veg%iveg(:))
  dummy(:) = casabiome%sla_grid(:)
...
!  REAL(r_2),    INTENT(IN) :: clitterinput(mp,3),csoilinput(mp,3)
  ...
  ! local variables
  INTEGER  npt,nout
  REAL(r_2) xyear, totGPP, totNPP, totNEP !added by xzhang 03/12/2014
...
!        clitterinput(npt,:),csoilinput(npt,:), &
      ...
      totNEP = totNEP+casabal%Fcneeyear(npt)* casamet%areacell(npt)
    ENDDO

    print *, 'totGPP global = ', totGPP*(1.0e-15)
    print *, 'totNPP global = ', totNPP*(1.0e-15)
    print *, 'totNEP global = ', totNEP*(1.0e-15) !added by xzhang 03/12/2014
  CLOSE(nout)
