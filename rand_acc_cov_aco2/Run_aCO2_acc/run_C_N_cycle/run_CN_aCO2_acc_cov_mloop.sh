...
ROOT=/home/bian_cy/case_unc/rand_acc_cov_aco2
WORKDIR=$ROOT/Run_aCO2_acc
DRIVER=$ROOT/CABLE2.0/offline/cable-mpi
OUTDIR=$WORKDIR/output/output_1xCO2
mkdir -p ${OUTDIR}
...
do
  ...
  # update nml for each loop
  rm nml
  cd ${WORKDIR}/nml_mloop/
  ./mknml_aCO2_acc_cov.bash ${i}
  cd ${original_dir}
  ln -sf ../nml_mloop nml

  # generate the outdir for each loop
  OUTDIR_loop=${OUTDIR}/out_aCO2_acc_cov_$i
  mkdir -p ${OUTDIR_loop}
  echo "mloop: ${i}, OUTDIR_loop: ${OUTDIR_loop}"
  ...
  do
    ...
    mv out_cable.nc      ${OUTDIR_loop}/out_cable_${i}_${yr}_aCO2_acc_cov.nc
    cp -p restart_out.nc restart_in.nc
    mv restart_out.nc    ${OUTDIR_loop}/restart_out_${i}_${yr}_aCO2_acc_cov.nc
