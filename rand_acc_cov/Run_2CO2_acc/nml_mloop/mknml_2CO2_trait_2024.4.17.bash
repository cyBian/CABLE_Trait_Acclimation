...
CO2=`awk '{if ($1=='${yr}') print$2}' global_2xco2_ann_1860_2013.out`
cat>${yr}/cable_init.nml<<EOF
&cable
   filename%met = ''
   ...
   filename%type    = 'CABLE-AUX/offline/gridinfo_CSIRO_CRU05x05.nc'
   filename%veg     = 'CABLE-AUX/core/biogeophys/veg_params_cable_MK3L_v3_X.Zhang.txt'
   ...
   output%patch = .FALSE.    ! do you use patch ?
   output%patchfrac = .FALSE.  !
   output%iveg = .TRUE.       !
   ...
   casafile%cnpmetout='casamet${yr}.nc'            ! output daily met forcing for spinning casacnp
   casafile%cnpmetin ='fcasamet.lst'          ! list of daily met files for spinning casacnp
   ...
   casafile%ndep         =''
   casafile%l_ndep       =.FALSE.
   ncciy = ${yr} ! 0 for not using gswp; 4-digit year input for year of gswp met
   globalMetfile%l_gpcc =.TRUE.
   globalMetfile%l_gswp =.FALSE.
   globalMetfile%l_ncar =.FALSE.
   globalMetfile%rainf = 'Trendy/prcp_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%snowf = ''
   globalMetfile%LWdown= 'Trendy/dlwrf_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%SWdown= 'Trendy/dswrf_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%PSurf = 'Trendy/pres_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%Qair  = 'Trendy/shum_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%Tair  = 'Trendy/tas_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%wind  = 'Trendy/wind_CRUNCEP_${yr}-${yr}.nc'
   globalMetfile%lai   = ''
...
&cable
   ...
   casafile%cnpbiome ='CABLE-AUX/core/biogeochem/pftlookup_csiro_v16_17tiles_JDLi.csv'  ! biome specific BGC parameters !changed by cybian 15 Nov, 2019
   casafile%cnpepool ='poolcnp_out.csv'       ! end of run pool size
   casafile%cnpmetout='casamet${yr}.nc'            ! output daily met forcing for spinning casacnp
   casafile%cnpmetin ='fcasamet.lst'          ! list of daily met files for spinning casacnp
   casafile%cnpspin      ='fcnpspin.lst'          ! list of cnp dump file for spinning up casacnp
   ...
   casafile%phen     ='CABLE-AUX/core/biogeochem/modis_phenology_csiro.txt'        ! modis phenology
   casafile%cnpflux  ='cnpflux${yr}.csv'
   casafile%ndep         ='/home/bian_cy/force_cable/Ndep_for_Trendy/ndep_${yr}_0.5x0.5.nc'
   casafile%l_ndep       =.TRUE.
   casafile%cnptrait = '/home/bian_cy/case_unc/0_data/SLA_Na_rand/SLA_Na_rand_0001.csv' !file for grid-based SLA and Nleaf (Bian,16Apr2024)
   ncciy = ${yr} ! 0 for not using gswp; 4-digit year input for year of gswp met
   globalMetfile%l_gpcc =.TRUE.
   globalMetfile%l_gswp =.FALSE.
   globalMetfile%l_ncar =.FALSE.
...
&end
EOF-run
cat>${yr}/cable_spincasa.nml<<EOF-spincasa
&cable
   filename%met = ''
   filename%out = 'out_cable.nc'
   filename%log = 'log_cable.txt'
   ...
   icycle = 2   ! BP pull it out from casadimension and put here; 0 for not using casaCNP, 1 for C, 2 for C+N, 3 for C+N+P
   casafile%cnpipool ='poolcnp_in.csv'       ! 
   casafile%cnpbiome ='CABLE-AUX/core/biogeochem/pftlookup_csiro_v16_17tiles_JDLi.csv'  ! biome specific BGC parameters
   casafile%cnpepool ='poolcnp_out.csv'       ! end of run pool size
   casafile%cnpmetout='casamet${yr}.nc'            ! output daily met forcing for spinning casacnp
   casafile%cnpmetin ='fcasamet.lst'          ! list of daily met files for spinning casacnp
   casafile%cnpspin      ='fcnpspin.lst'          ! list of cnp dump file for spinning up casacnp
...
&end
EOF-spincasa
cat>${yr}/cable_dump.nml<<EOF-dump
&cable
   filename%met = ''
   filename%out = 'out_cable.nc'
   filename%log = 'log_cable.txt'
   ...
   l_laiFeedbk   = .TRUE.  ! using prognostic LAI
   l_vcmaxFeedbk = .TRUE.  ! using prognostic Vcmax
   icycle = 3   ! BP pull it out from casadimension and put here; 0 for not using casaCNP, 1 for C, 2 for C+N, 3 for C+N+P
   casafile%cnpipool ='poolcnp_in.csv'       ! 
   casafile%cnpbiome ='CABLE-AUX/core/biogeochem/pftlookup_csiro_v16_17tiles_JDLi.csv'  ! biome specific BGC parameters
   casafile%cnpepool ='poolcnp_out.csv'       ! end of run pool size
   casafile%cnpmetout='casamet${yr}.nc'            ! output daily met forcing for spinning casacnp
